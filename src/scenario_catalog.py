"""The PuffSat scenario catalog (CONTEXT.md "Scenarios").

The ordered list of the paper's PuffSat scenarios and the top-level analysis
functions that don't belong to any one re-intercept or growth-loop leg:
lunar-return optimization, launch-capacity timing, and the standalone solar-
periapsis / suborbital figures the paper cites directly. Combines the
heliocentric re-intercept (heliocentric_reintercept.py) and apoapsis-raise
re-intercept (apoapsis_raise_reintercept.py) legs into the phased Earth-return
sibling catalog; it does not yet fold in the Jovian powered-flyby or
unpowered-assist-chain results, which the catalog rows report separately at
the paper's published retrograde-Hohmann v_b (see CONTEXT.md "Jupiter
powered-flyby retrograde return").
"""
