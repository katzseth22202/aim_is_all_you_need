...
scaling, the break-even algebra, and the impulse quadrature, which has an exact
answer on a circular orbit.  The slow ones fly the chains, which is where the
ADR 0026 figures come from.
"""
...
from src.sep_split_correction import SEP_ARRAY_POWER_1AU_KW
from src.sep_split_correction import SEP_ARRAY_POWER_1AU_KW as _P0
from src.sep_split_correction import (
...
def test_the_analysis_tables_line_up_with_the_flown_chains():
    ...
    assert set(analysis.chains.propellant) == {"methalox", "argon"}
    assert len(analysis.chains) == len(CADENCE_POLICIES) * 4
...
def test_deliverable_dv_rescales_linearly_with_acceleration():
    ...
    reference = characteristic_acceleration(_P0, 1.0)
